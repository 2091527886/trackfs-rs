...
const FLAC_CRC16_TABLE: [[u16; 256]; 8] = [
    [
...
pub fn crc16(data: &[u8]) -> u16 {
    data.chunks(8).fold(0, |crc, data_chunk| {
        if data_chunk.len() == 8 {
            let crc = crc ^ ((data_chunk[0] as u16) << 8 | data_chunk[1] as u16);

            FLAC_CRC16_TABLE[7][(crc >> 8) as usize]
                ^ FLAC_CRC16_TABLE[6][(crc & 0xFF) as usize]
                ^ FLAC_CRC16_TABLE[5][data_chunk[2] as usize]
                ^ FLAC_CRC16_TABLE[4][data_chunk[3] as usize]
                ^ FLAC_CRC16_TABLE[3][data_chunk[4] as usize]
                ^ FLAC_CRC16_TABLE[2][data_chunk[5] as usize]
                ^ FLAC_CRC16_TABLE[1][data_chunk[6] as usize]
                ^ FLAC_CRC16_TABLE[0][data_chunk[7] as usize]
        } else {
            data_chunk.iter().fold(crc, |crc, data_byte| {
                (crc << 8) ^ FLAC_CRC16_TABLE[0][((crc >> 8) ^ (*data_byte) as u16) as usize]
            })
        }
    })
}
