...
pub enum CUEParseError {
    ...
    /// track
    #[error("missing {0} entry in cue track: ")]
...
impl TryFrom<&str> for CUEFile {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
                    ...
                    "TITLE" => {
                        title = Some(
                            line_split
                                .next()
                                .map(|s| {
                                    let trimmed = s.trim_surrounding('"');
                                    if trimmed.is_empty() {
                        ...
                        performer = Some(
                            line_split
                                ...
                                .map(|s| {
                                    let trimmed = s.trim_surrounding('"');
                                    if trimmed.is_empty() {
                                        "Unknown Performer".to_owned() // 空字符串处理
                                    } else {
                                        trimmed.to_owned() // 有效值保留
                                    }
                                })
                                .unwrap_or_else(|| "Unknown Performer".to_owned()) // 处理无值情况
                        );
                    }
                    "CATALOG" => {
                        // 获取原始值（允许空值）
                        let raw_val = line_split.next().unwrap_or("").trim();
                        
                        // 验证并生成最终值
                        let catalog_val = if raw_val.len() == 13 
                            && raw_val.chars().all(|c| c.is_ascii_digit()) 
                        {
                            raw_val.to_owned()  // 有效值保留
                        } else {
                            "0000000000000".to_owned()  // 无效值替换
                        };
                        
                        catalog = Some(catalog_val);  // 确保始终有值
                    }
                    "CDTEXTFILE" => {
                        text_file = Some(
                            line_split
        ...
        if files.is_empty() {
            Err(CUEParseError::MissingEntry("file"))?
        }
        let title = title.unwrap_or_else(|| "Unknown Title".to_owned());
        let performer = performer
            .map(|s| {
                // 处理空字符串的情况（例如 PERFORMER ""）
                if s.is_empty() {
                    "Unknown Performer".to_owned()
                } else {
                    s.to_owned() // 原 map 逻辑保留
                }
            })
            .unwrap_or_else(|| "Unknown Performer".to_owned());
